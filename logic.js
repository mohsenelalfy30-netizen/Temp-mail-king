    ...
    try {
        ...
        if (messages.length === 0) {
            ...
            htmlContent += messages.map((msg) => {
                ...
                // تنظيف الهيدرز التقنية المزعجة بشكل شامل
                if (rawBody.includes('DKIM-Signature:') || rawBody.includes('Received:') || rawBody.includes('Content-Type:')) {
                    const parts = rawBody.split(/\r?\n\r?\n/);
                    if (parts.length > 1) {
                        parts.shift(); // إزالة الهيدرز
                        rawBody = parts.join('\n\n'); 
                    }
                }
                ...
                function decodeMassive(text) {
                    try {
                        // معالجة تشفير base64 بشكل مباشر إذا وجد
