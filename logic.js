...
const libraryList = document.getElementById('libraryList'); // تمت الإضافة لربط القائمة
...
function renderLibrary() {
    ...
    // --- بناء واجهة المكتبة وإضافة زر الحذف لكل إيميل ---
    if (libraryList) {
        if (data.length === 0) {
            ...
            libraryList.innerHTML = data.map(item => `
                <div class="flex items-center justify-between p-3 bg-white dark:bg-darkCard border border-gray-200 dark:border-darkBorder rounded-xl shadow-sm hover:border-iosBlue transition-colors group">
                    <button onclick="window.loadFromLibrary('${item.email}')" class="flex-1 text-start font-mono text-sm text-black dark:text-white truncate">
                        ${item.email}
                    </button>
                    <button onclick="window.deleteEmailRecord('${item.email}')" class="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition-colors shrink-0 ms-2" title="حذف">
...
window.deleteEmailRecord = function(targetEmail) {
    ...
    if (currentActiveEmail === targetEmail || currentActiveRealEmail === targetEmail) {
        resetUIForEmptyState();
        // If there are other emails, show the most recent one
        if (savedEmails.length > 0) {
            showEmail(savedEmails[0].email);
        }
    }
...
// ==========================================
// NEW: Advanced & Smart Decoding Engine
// ==========================================
function advancedDecode(text) {
    if (!text) return "";
    let decodedText = text;

    // 1. Quoted-Printable Decoding (=3D and line breaks)
    if (decodedText.includes('=3D') || decodedText.includes('=\r\n') || decodedText.includes('=\n')) {
        decodedText = decodedText.replace(/=\r?\n/g, ''); // Remove soft line breaks
        decodedText = decodedText.replace(/=3D/gi, '=');  // Fix equal signs
        
        // Decode encoded UTF-8 hex (e.g., =D8=A7 for Arabic)
    ...
    // 2. Base64 Extraction & Decoding
    if (decodedText.includes('base64')) {
        const parts = decodedText.split(/\r?\n\r?\n/);
        for (let i = 0; i < parts.length; i++) {
            let part = parts[i];
            // Identify base64 block (no colon headers, enough length, correct chars)
...
function renderMessagesHTML(messages) {
    ...
    if (messages.length === 0) {
        ...
        htmlContent += messages.map((msg) => {
            ...
            // Allow essential styling and table tags to support complex emails like Gmail formats
