    ...
    createBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.exists) {
                const toast = document.createElement('div');
                toast.className = 'fixed bottom-4 left-1/2 -translate-x-1/2 bg-red-600 text-white px-4 py-2 rounded-lg text-sm shadow-lg z-50 transition-opacity duration-300';
                // إضافة الترجمة هنا للإيميل المستخدم
...
// --- دالة جلب وبناء واجهة الرسائل (مع الفلترة الشاملة للكلام الأسود وحل مشكلة التشفير) ---
        ...
        if (messages.length === 0) {
            ...
            htmlContent += messages.map((msg) => {
                ...
                if (rawBody.match(/^[A-Za-z0-9-]+:/m) || rawBody.includes('DKIM-Signature:') || rawBody.includes('Received:')) {
                    ...
                    if (parts.length > 1) {
                        // البحث عن أول جزء لا يحتوي على ترويسات تقنية، أو أخذ المحتوى بالكامل بعد السطر الفارغ
