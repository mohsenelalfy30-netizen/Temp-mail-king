...
let isDarkMode = true;
let savedEmails = [];
let activePollTimeout = null; // الموقت الذكي الجديد
...
// --- Visibility API لتعطيل الفحص أثناء الخمول ---
...
// --- دالة العرض المتطورة مع التحديث الذكي ---
async function showEmail(email, isReRender = false) {
    ...
    if (!isReRender) {
        await fetchAndRenderMessages(realEmail, email);
...
function scheduleNextPoll(realEmail, emailKey) {
    ...
    if (!saved) return; // تم حذفه

    // شرط التوقف: الصندوق ممتلئ
    if (saved.messages && saved.messages.length >= 2) return;

    const elapsed = Date.now() - saved.createdAt;
    const TWENTY_MINS = 20 * 60 * 1000;
    
    if (elapsed >= TWENTY_MINS) return; // انتهت الصلاحية

    let delay;
    if (elapsed < 60000) {
        delay = 15000; // أول دقيقة: كل 15 ثانية
    } else if (elapsed < 10 * 60 * 1000) {
        delay = 120000; // الدقائق 2-10: كل دقيقتين
    } else {
        delay = 300000; // الدقائق 10-20: كل 5 دقائق
    }
...
// --- دالة رسم الواجهة (مفصولة لتنظيف الكود) ---
function renderMessagesHTML(messages) {
    ...
    if (messages.length === 0) {
        ...
        htmlContent += messages.map((msg) => {
            ...
            if (rawBody.match(/^[A-Za-z0-9-]+:/m) || rawBody.includes('DKIM-Signature:') || rawBody.includes('Received:')) {
                const parts = rawBody.split(/\r?\n\r?\n/);
                if (parts.length > 1) {
                    rawBody = parts.find(p => !p.match(/^[A-Za-z0-9-]+:/)) || parts.slice(1).join('\n\n'); 
                }
            }

            function cleanAndDecode(text) {
                try {
                    if (text.includes('base64')) {
                        const b64Part = text.split('base64')[1] || text;
                        try { return decodeURIComponent(escape(atob(b64Part.replace(/\s/g, '')))); } catch(e){}
                    }
                    if (text.includes('=')) {
                    ...
                    text = text.replace(/Content-(Type|Transfer-Encoding|Id|Description|Disposition):[^\n]*\n/gi, '');
            ...
            let processedBody = cleanAndDecode(rawBody).trim();
            const safeHTML = (window.DOMPurify) ? DOMPurify.sanitize(processedBody) : processedBody;

            let otpMatch = null;
            if (msg.subject) otpMatch = msg.subject.match(/\b\d{4,8}\b/);
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl mb-3 overflow-hidden shadow-sm hover:shadow-md transition-all">
                ...
                <div class="hidden p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
                    ...
                    <div class="message-content text-sm text-gray-700 dark:text-gray-300 overflow-x-auto leading-relaxed" style="word-break: break-word;">
                        ${safeHTML.includes('<table') || safeHTML.includes('<div') ? safeHTML : `<pre class="whitespace-pre-wrap font-sans">${safeHTML}</pre>`}
                    </div>
                </div>
            </div>`;
        }).join('');
