...
function advancedDecode(text) {
    ...
    let cleanText = text.replace(/^[\s\S]*?Content-Transfer-Encoding: base64\r?\n\r?\n/i, "");
    cleanText = cleanText.replace(/Content-Type:[\s\S]*?(?=\r?\n\r?\n)/gi, "");

    if (/^[A-Za-z0-9+/=\s\n\r]+$/.test(cleanText.trim()) && cleanText.length > 20) {
        try {
            return decodeURIComponent(escape(atob(cleanText.replace(/\s/g, ''))));
        } catch (e) {
            try { return atob(cleanText.replace(/\s/g, '')); } catch(e2) { return text; }
        }
    }
    return text;
}
    ...
    if (messages.length === 0) {
        ...
        htmlContent += messages.map((msg) => {
            ...
            const safeHTML = (window.DOMPurify) ? DOMPurify.sanitize(processedBody, {
                ALLOWED_TAGS: ['b', 'i', 'em', 'strong', 'a', 'p', 'br', 'ul', 'li', 'ol', 'img', 'div', 'span', 'table', 'tr', 'td', 'tbody', 'h1', 'h2', 'h3', 'pre', 'code'],
                ALLOWED_ATTR: ['href', 'src', 'style', 'target', 'alt', 'width', 'height', 'class']
            }) : processedBody;
