...
function cleanupOldAccounts() {
    ...
    try {
        if (savedEmails.length > 0) {
            ...
            // إذا تم حذف إيميلات، نقوم بتحديث المكتبة
            if (savedEmails.length !== initialLength) {
                localStorage.setItem('tempMailLibrary', JSON.stringify(savedEmails));
                renderLibrary();
                
                // إيقاف التحديث التلقائي إذا انتهت صلاحية جميع الإيميلات
                if (savedEmails.length === 0 && inboxPollingInterval) {
...
if (createBtn) {
    createBtn.addEventListener('click', async () => {
        ...
        // --- التعديل: فحص التوفر في المخزن (Cloudflare KV) لمنع التداخل ---
        try {
            createBtn.disabled = true;
...
// --- دالة جلب وبناء واجهة الرسائل (مع التنبيهات المخصصة) ---
async function fetchAndRenderMessages(realEmail) {
    // التوافق مع أي من الأسماء المستخدمة في HTML الخاص بك
    const container = document.getElementById('inboxContainer') || messagesList;
    if (!container) return;

    try {
        const data = await fetchInbox(realEmail);
        // التوافق مع حالة عودة الـ API كمصفوفة مباشرة أو ككائن
        const messages = Array.isArray(data) ? data : (data.messages || []);

        let htmlContent = '';

        // التعديل: رسالة التنبيه إذا الصندوق ممتلئ
        if (messages.length >= 2) {
            htmlContent += `
                <div class="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl text-center shadow-sm">
                    ...
                    <h3 class="text-red-800 dark:text-red-400 font-bold text-sm mb-1">الصندوق ممتلئ (2/2)</h3>
                    <p class="text-xs text-red-600 dark:text-red-300">لقد استلمت الحد الأقصى للرسائل. للحفاظ على الأمان والتنظيم، يرجى إنشاء إيميل جديد لاستقبال رسائل إضافية.</p>
                </div>
            `;
        }
        ...
        if (messages.length === 0) {
            htmlContent += `
                <div class="flex flex-col items-center justify-center py-12 text-gray-500">
                    ...
                    <p>${i18n.t ? i18n.t('no_messages') : (i18n.getTranslation('no_messages') || 'لا توجد رسائل')}</p>
                </div>`;
        } else {
            messages.sort((a, b) => b.timestamp - a.timestamp);

            htmlContent += messages.map((msg) => {
                // لضمان سحب المحتوى مهما كان اسم المتغير في הـ API
...
if (refreshInboxBtn) {
    refreshInboxBtn.addEventListener('click', async () => {
        ...
        // Fetch new messages (Re-render mode)
