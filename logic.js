...
async function init() {
    ...
    i18n.onLanguageChange(() => {
        ...
        // إجبار واجهة الرسائل على التحديث لعرض الترجمة الجديدة فوراً
        const container = document.getElementById('inboxContainer') || messagesList;
...
if (createBtn) {
    createBtn.addEventListener('click', async () => {
        ...
        // --- 1. تم نقل منطق الإعلان هنا للأعلى لضمان الفتح الفوري بدون حظر ---
        try {
        ...
        // --- 3. إضافة رسالة التنبيه بمدة 20 دقيقة (مربوطة بنظام الترجمة) ---
        ...
        // جلب الترجمة هنا بناءً على المفتاح expiry_warning
        ...
        setTimeout(() => {
            expireToast.classList.remove('-translate-y-20', 'opacity-0');
        }, 50);
    ...
    try {
        ...
        if (messages.length === 0) {
            ...
            htmlContent += messages.map((msg) => {
                ...
                // 1. فلترة الترويسات التقنية (Smart Header Stripping)
                ...
                // --- 2. إضافة DOMPurify.sanitize واستخدامه بشكل آمن ---
                // تأكد من تضمين مكتبة DOMPurify في ملف HTML الخاص بك 
