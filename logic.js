...
// ==========================================
// UPDATED: Professional Multipart Decoding Engine
// ==========================================
function advancedDecode(text) {
    if (!text) return "";
    let decodedText = text;

    // 1. التعامل مع الرسائل المتعددة الأجزاء (Multipart) - شائعة جداً في Gmail
    const boundaryMatch = decodedText.match(/boundary="?([^"\s;]+)"?/i);
    if (boundaryMatch) {
        const boundary = boundaryMatch[1];
        const parts = decodedText.split('--' + boundary);
        
        // البحث عن جزء الـ HTML أولاً، ثم النص العادي كبديل
        let selectedPart = parts.find(p => p.toLowerCase().includes('content-type: text/html')) || 
                           parts.find(p => p.toLowerCase().includes('content-type: text/plain')) || 
                           parts[1]; 

        if (selectedPart) {
            const bodySplit = selectedPart.split(/\r?\n\r?\n/);
            if (bodySplit.length > 1) {
                const partHeaders = bodySplit[0];
                let partBody = bodySplit.slice(1).join('\n\n').trim();
                partBody = partBody.replace(/--\s*$/, ''); // تنظيف نهاية الجزء

                // فك تشفير الجزء المختار بناءً على نوعه
                if (/content-transfer-encoding:\s*base64/i.test(partHeaders)) {
                    try {
                        let cleanBase64 = partBody.replace(/\s+/g, '');
                        let b64Decoded = atob(cleanBase64);
                        try { partBody = decodeURIComponent(escape(b64Decoded)); } catch(e){ partBody = b64Decoded; }
                    ...
                    partBody = partBody.replace(/(?:=[0-9A-F]{2})+/gi, (match) => {
                        try { return decodeURIComponent(match.replace(/=/g, '%')); } catch(e) { return match; }
                    });
                }
                return partBody;
            }
    ...
    // 2. التعامل مع الرسائل العادية (غير المقسمة)
    if (decodedText.includes('=3D') || decodedText.includes('=\r\n')) {
        decodedText = decodedText.replace(/=\r?\n/g, '').replace(/=3D/gi, '=');
        decodedText = decodedText.replace(/(?:=[0-9A-F]{2})+/gi, m => {
            try { return decodeURIComponent(m.replace(/=/g, '%')); } catch(e) { return m; }
        });
    }
