...
let isDarkMode = true;
let savedEmails = [];
let inboxPollingInterval = null; // تمت الإضافة: للتحكم في التحديث التلقائي
    ...
    try {
        if (savedEmails.length > 0) {
            ...
            if (startTime && (Date.now() - startTime >= TWENTY_FOUR_HOURS)) {
                ...
                // تمت الإضافة: إيقاف التحديث التلقائي إذا انتهت الـ 24 ساعة
...
// --- تمت الإضافة/التعديل: دالة العرض المتطورة مع التحديث التلقائي ---
async function showEmail(email, isReRender = false) {
    ...
    // المنطق الجديد: التحديث التلقائي وجلب الرسائل
    if (!isReRender) {
        // إذا كنا نفتح إيميلاً جديداً أو ننتقل إليه، نقوم بإيقاف أي مراقبة سابقة
        if (inboxPollingInterval) {
            clearInterval(inboxPollingInterval);
        }
        
        // جلب وعرض الرسائل فوراً
        await fetchAndRenderMessages(realEmail);
        
        // تشغيل المراقبة كل ثانيتين
        inboxPollingInterval = setInterval(() => {
            fetchAndRenderMessages(realEmail);
        }, 2000);
    } else {
        // إذا كان مجرد تحديث يدوي (Refresh)
        await fetchAndRenderMessages(realEmail);
    }
}
